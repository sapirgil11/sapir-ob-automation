...
export class EmailVerificationFlow {
  ...
  async testTypeCorrectMfaAndVerifyNavigation(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `❌ Error in testTypeCorrectMfaAndVerifyNavigation: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async testInlineErrors(): Promise<boolean> {
    ...
    try {
      ...
      const mfaOtpInvalidErrorVisible =
        await this.emailVerificationPage.mfaOtpInvalidError.isVisible();

      console.log(`❌ MFA OTP Error: ${mfaOtpErrorVisible ? '✅ Visible' : '❌ Not visible'}`);
      console.log(
        `❌ MFA OTP Invalid Error: ${mfaOtpInvalidErrorVisible ? '✅ Visible' : '❌ Not visible'}`
      );
      ...
      console.log(
        `❌ Error in testInlineErrors: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async testElementsExist(): Promise<boolean> {
    ...
    try {
      ...
      const coreElements = [
        {
          ...
          locator: this.emailVerificationPage.verificationCodeInput,
          required: true,
        },
        {
          name: 'Resend Code Button',
          locator: this.emailVerificationPage.resendCodeButton,
          required: false,
        },
        {
          name: 'Error Container',
          locator: this.emailVerificationPage.errorContainer,
          required: false,
        },
      ];
      ...
      const titleElements = [
        {
          name: 'Verification Heading (Title)',
          locator: this.emailVerificationPage.verificationHeading,
          required: true,
        },
        {
          name: 'Email Display (Subtitle)',
          locator: this.emailVerificationPage.emailDisplay,
          required: true,
        },
        {
          name: 'Enter Code Text',
          locator: this.emailVerificationPage.enterCodeText,
          required: false,
        },
      ];
      ...
      const errorElements = [
        ...
        {
          name: 'MFA OTP Invalid Error',
          locator: this.emailVerificationPage.mfaOtpInvalidError,
          required: false,
        },
        {
          name: 'MFA OTP Expired Error',
          locator: this.emailVerificationPage.mfaOtpExpiredError,
          required: false,
        },
      ];
  ...
  async testResendCode(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `📋 Resend OTP Button: ${isResendButtonVisible ? '✅ Visible' : '❌ Not visible'}`
      );
      ...
      console.log(
        `❌ Error in testResendCode: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
