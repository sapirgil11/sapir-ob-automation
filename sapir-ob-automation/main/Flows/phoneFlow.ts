...
export class PhoneFlow {
  ...
  async testFillPhoneForm(emailPrefix?: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `❌ Error navigating to Phone page: ${
  ...
  async fillPhoneForm(phoneNumber?: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `❌ Error filling phone form: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async testTypePhoneNumberWithoutDropdown(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `❌ Error in testTypePhoneNumberWithoutDropdown: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return false;
  ...
  async testDropdownSearchAndFill(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `❌ Error in testDropdownSearchAndFill: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return false;
  ...
  async testValidationErrorsFlow(): Promise<boolean> {
    console.log(
      '🧪 Testing: Validation Errors Flow - 123 > Error > Clear > 22222 > Inline Error > Clear > Valid > Next Page'
    );
    ...
    try {
      ...
      console.log(
        '🔄 Trying approach: 123 > click outside frame > click input > unfocus (repeat 3 times)...'
      );
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        console.log(
          `  ❌ Short phone error visible (attempt ${attempt}): ${shortErrorVisible ? '✅' : '❌'}`
        );
      ...
      if (!shortErrorVisible) {
        ...
        shortErrorVisible = await this.phonePage.isPhoneNumberErrorVisible();
        console.log(
          `❌ Short phone error visible (continue button): ${shortErrorVisible ? '✅' : '❌'}`
        );
      }
      ...
      console.log(
        `\n🎯 Validation errors flow test: ${
          allStepsPassed ? '✅ ALL STEPS PASSED' : '❌ SOME STEPS FAILED'
        }`
      );
      return allStepsPassed;
    } catch (error) {
      console.log(
        `❌ Error in testValidationErrorsFlow: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      return false;
