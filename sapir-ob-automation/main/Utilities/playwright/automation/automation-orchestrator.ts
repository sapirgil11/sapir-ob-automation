...
export class AutomationOrchestrator {
  ...
  constructor(environment?: string) {
    ...
    console.log(
      `🚀 Automation Orchestrator initialized for environment: ${this.currentEnvironment}`
    );
  }
  ...
  async runSmokeTests(): Promise<TestReport> {
    ...
    try {
      ...
      const smokeResults = results.filter(
        r => r.testFile.includes('welcomeScreenTest') || r.testFile.includes('loginPage')
      );
