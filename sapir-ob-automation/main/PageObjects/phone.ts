...
export class Phone {
  ...
  private initializePageContent(): void {
    ...
    this.pageSubheading = this.page.locator(
      "text:has-text('Your account will be protected using Two-Factor Authentication with your mobile number.')"
    );
  }
  ...
  private initializeErrorElements(): void {
    ...
    this.phoneNumberExistsError = this.page.locator('text', {
      hasText: "Currently, we aren't able to open an account for this number",
    });
    this.phoneNumberInvalidApiError = this.page.locator(
      'text=Are you sure this is your mobile phone number?'
    );
  }
