...
test.describe('🏢 Business Type Page Flow Tests', () => {
    ...
    test('🔍 Text and Subtitle Verification Test', async ({ page }) => {
        ...
        const mainTitle = await page.getByRole('heading', { name: 'Select your business type' }).isVisible();
        console.log(`✅ Main page title "Select your business type": ${mainTitle ? 'FOUND' : 'NOT FOUND'}`);
        ...
        const generalPartnershipButton = await page.locator('#business-sub-type-general_partnership').isVisible();
        const llpButton = await page.locator('#business-sub-type-llp').isVisible();
        console.log(`✅ General Partnership button: ${generalPartnershipButton ? 'FOUND' : 'NOT FOUND'}`);
