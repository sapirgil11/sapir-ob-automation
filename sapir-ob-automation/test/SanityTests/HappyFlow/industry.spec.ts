...
test.describe('🏭 Industry Page Flow Tests', () => {
    ...
    test('🎯 Industry Page Tooltip Verification Test', async ({ page }) => {
        ...
        const tooltipNote = page.locator('p.text-\\[14px\\].leading-\\[20px\\].font-metricR.text-text-main-secondary');
        ...
        const tooltipContent = page.locator('[data-tooltip-id="tooltip-banned-activities"] + div, [role="tooltip"]');
        ...
        if (isTooltipVisible) {
            ...
            console.log(`📊 Found ${foundActivities}/${expectedActivities.length} expected activities in tooltip`);
            expect(foundActivities).toBeGreaterThan(0);
            
        } else {
            console.log('⚠️ Tooltip content not immediately visible, checking for alternative selectors...');
