...
test.describe('🧾 Know Your Business (KYB) Page Tests', () => {
    ...
    async function submitWithEinRetry(page: any, kyb: KnowYourBusiness, initialEin: string, maxAttempts = 3) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            const serverError = page.getByText('Please verify that the number matches the one on your IRS record');
    ...
    test('🎉 KYB - Happy Path (randomized data) and Continue', async ({ page }) => {
        ...
        console.log(`🧾 Using Business Name: ${result.name}, EIN: ${result.ein}, State: ${result.state}`);
    ...
    test('❌ KYB - Inline Errors (empty fields, invalid EIN)', async ({ page }) => {
        ...
        await expect(page.locator('#BUSINESS_NAME-error-container')).toContainText('Business name is required');
        ...
        await page.click('body');
        await expect(page.locator('#BUSINESS_NAME-error-container')).toBeVisible();
        await expect(page.locator('#BUSINESS_NAME-error-container')).toContainText('Legal business name must be 100 characters or less');
        await kyb.clearBusinessName();
        ...
        await page.click('body');
        await expect(page.locator('#BUSINESS_NAME-error-container')).toBeVisible();
        await expect(page.locator('#BUSINESS_NAME-error-container')).toContainText('Business must be at least two letters');
    ...
    test('🔍 KYB - Elements Exist and Clear Buttons', async ({ page }) => {
        ...
        await expect(page.getByText('Please provide your business information as it appears in your legal documents.')).toBeVisible();
        ...
        await expect(page.getByText("I understand that by clicking 'continue' I certify that I have significant responsibility to control, manage and direct the business")).toBeVisible();
